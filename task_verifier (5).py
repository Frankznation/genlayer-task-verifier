...
=====================================
A GenLayer Intelligent Contract that uses AI validators to verify
real-world task completion through evidence analysis.
"""
...
class TaskVerifier(gl.Contract):
    ...
    # State variables - must use GenLayer types (u256 for integers)
    task_counter: u256
    tasks: TreeMap[u256, str]
    
    def __init__(self):
        """Initialize the contract with zero tasks."""
    ...
    def create_task(
        ...
        """
        Create a new task with AI verification criteria.
        """
    ...
    def claim_task(self, task_id: int) -> bool:
        ...
        task["status"] = "claimed"
        task["worker"] = str(gl.message.sender)
    ...
    def submit_proof(self, task_id: int, proof_url: str) -> bool:
        """
        Submit proof of task completion for AI verification.
        """
        key = u256(task_id)
        if key not in self.tasks:
            return False
        ...
        if task["worker"] != str(gl.message.sender):
            return False
    ...
    def verify_completion(self, task_id: int) -> str:
        """
        Trigger AI verification of submitted proof.
        """
        key = u256(task_id)
        if key not in self.tasks:
        ...
        # Step 1: Fetch evidence from proof URL
        def fetch_evidence():
        ...
        if not evidence or len(evidence) < 50:
            ...
            task["result"] = "Could not access proof URL or insufficient content"
            self.tasks[key] = json.dumps(task)
...
Based on the evidence provided, determine if the task has been completed according to the verification criteria.

Respond with EXACTLY one of these two formats (no other text):
VERIFIED: [brief explanation of how criteria were met]
OR
REJECTED: [brief explanation of what criteria were not met]"""
        ...
        final_result = gl.eq_principle_prompt_non_comparative(
            analyze_evidence,
            task=f"Verify if evidence meets criteria for: {title}",
    ...
    def get_task(self, task_id: int) -> str:
        """Get all details of a specific task as JSON."""
        key = u256(task_id)
        if key not in self.tasks:
            return "{}"
